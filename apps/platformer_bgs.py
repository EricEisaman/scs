# apps/platformer_bgs.py - V4.5 - DIRECTION-DEPENDENT TRAIL ORIGIN
# Examined: trail previously started from center (p.x, p.y), not direction-dependent
# Fix: trail now starts from trailing edge, based on facing / velocity direction
...
class RemoteVisual:
    def __init__(self, clientId, color):
        ...
        self.color = color
        self.trail = []  # List[(x,y)] in world space, origin = trailing edge
        self.facing = 1
...
class World:
    ...
    def step(self):
        ...
        for p in list(self.players.values()):
            ...
            # DIRECTION-DEPENDENT TRAIL ORIGIN - FIXED
            # Previously: trail from center (p.x, p.y) - not direction dependent
            # Now: trail from trailing edge, opposite to facing
            # If facing=1 (right), trail starts from left side: x - w*0.4
            # If facing=-1 (left), trail starts from right side: x + w*0.4
            # Y offset at feet for ground dust effect
            try:
                facing = int(p.facing) if p.facing!=0 else 1
                trail_origin_x = float(p.x) - float(facing) * (float(p.w) * 0.45)
                trail_origin_y = float(p.y) + float(p.h) * 0.25
                # Only add to trail if actually moving, to avoid blob when standing
                speed = math.hypot(float(p.vx), float(p.vy))
                if speed > 0.5 or abs(float(p.vx)) > 0.5:
                    p.trail.append((trail_origin_x, trail_origin_y))
                else:
                    # slowly fade trail when standing
                    if p.trail and random.random() < 0.3:
                        p.trail.pop(0)
            except Exception as e:
                # fallback to center
            ...
            for coin in self.coins:
                ...
                if abs(dx)<22 and abs(dy)<28:
                    coin["collected"]=True; p.score+=coin.get("val",10); p.coin_flash=10
        for coin in self.coins:
...
def onAppStart(app):
    app.width=1050; app.height=700; app.stepsPerSecond=60; app.background=rgb(15,15,30)
            ...
            try:
                ...
                if not cid:
                    try:
                        ...
                        cid=d.get("client_id")
                    except: cid=getattr(res, "client_id", None)
                app.client_id=cid; print(f"[BGS] Joined {app.client_id}")
...
def onKeyPress(app, key):
    if key=='r':
        ...
        app.remote_players.clear(); app.remote_visuals.clear()
    ...
    if key=='g':
        ...
        print(f"[BGS] Trails {'ON' if app.enable_trails else 'OFF'} - origin is direction-dependent")
...
def onStep(app):
    ...
    for cid, remote in list(app.remote_players.items()):
        ...
        # Previously: trail from center (px, py)
        # Now: from trailing edge opposite to motion direction
        # If moving right (vx>0, facing=1), trail should come from left side: px - 15
        # If moving left (vx<0, facing=-1), trail should come from right side: px + 15
        try:
            facing = int(vis.facing) if vis.facing!=0 else 1
            trail_origin_x = px - float(facing) * 15.0  # 15 = half width of remote rect
            trail_origin_y = py + 10.0  # feet offset
            speed = math.hypot(vx, vy)
            if speed > 0.5:
                vis.trail.append((trail_origin_x, trail_origin_y))
            else:
    ...
    if HAS_MP and app.mp_client and app.client_id and "local_0" in app.world.players:
        ...
        if now - app.last_send_ms > 100:
            ...
            try:
                safe_x = float(lp.x); safe_y = float(lp.y); safe_vx = float(lp.vx); safe_vy = float(lp.vy)
            ...
            except Exception as e: print(f"[BGS] send fail {e}")

def drawTrail(trail, color, camera_x, is_local=False, facing=1):
    if not trail or len(trail)<2: return
    for i in range(len(trail)-1):
        ...
        if dist < 1 and t < 0.8: continue
        line_width = 2 + t * 10
        alpha = 0.15 + t * 0.85
        # Add directional arrow effect - trail fades opposite to motion
        if is_local and t > 0.7:
            try: drawLine(x1, y1, x2, y2, fill=rgb(255,255,255), lineWidth=line_width+2, opacity=alpha*0.3)
            except: pass
        try: drawLine(x1, y1, x2, y2, fill=color, lineWidth=line_width, opacity=alpha)
        except:
            try: drawLine(x1, y1, x2, y2, fill=color, lineWidth=line_width)
            except: pass
        circle_size = 3 + t * 10
        try: drawCircle(x1, y1, circle_size, fill=color, opacity=alpha*0.6)
        except:
            try: drawCircle(x1, y1, circle_size, fill=color)
            except: pass
...
def redrawAll(app):
    ...
    for pid, p in app.world.players.items():
        ...
        if app.enable_trails and p.trail:
            # Pass facing so trail knows direction
            drawTrail(p.trail, p.color, app.camera_x, is_local=True, facing=int(p.facing))
            # Debug: show trail origin vs center
            # drawCircle(x - int(p.facing)*14, y+11, 3, fill=rgb(255,0,0)) # red dot = origin
        col=p.color
    ...
    for cid in list(app.remote_players.keys()):
        ...
        vis = app.remote_visuals.get(cid)
        if not vis: continue
        if app.enable_trails and vis.trail and len(vis.trail)>1:
    ...
    status=f"BGS-MP-SYNC | LOCAL: {len(app.world.players)} | REMOTE: {len(app.remote_players)} | TRAILS: {'ON' if app.enable_trails else 'OFF'} (G) | DIR-DEPENDENT"
    ...
    if app.remote_players:
        ...
        drawLabel("REMOTE (BGS)",90,y,size=11,fill=rgb(180,200,255)); y+=14
        for cid, r in list(app.remote_players.items())[:6]:
            st=r.get("animationState","?"); drawLabel(f"{cid[:6]} {st}",90,y,size=10,fill=rgb(180,180,255)); y+=16

    if app.show_help:
        hx=app.width-160; hy=80
        drawRect(hx,hy+60,300,150,fill=rgb(0,0,0))
        ...
        drawLabel("Trail origin: trailing edge",hx,hy+32,size=10,fill=rgb(255,235,100))
        drawLabel("facing=1 (right) -> x - w",hx,hy+48,size=10,fill=rgb(180,200,255))
        drawLabel("facing=-1 (left) -> x + w",hx,hy+64,size=10,fill=rgb(180,200,255))
        drawLabel(f"Len: {app.trail_length}",hx,hy+80,size=10,fill=rgb(255,235,100))

    drawRect(app.width//2,app.height-18,app.width,36,fill=rgb(0,0,0))
    drawLabel("BGS-MP-SYNC: direction-dependent trail origin | server: scs-datastar-extension",app.width//2,app.height-18,size=9,fill=rgb(120,255,180))
